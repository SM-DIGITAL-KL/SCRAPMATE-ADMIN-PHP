                    ...
                    <div class="card-body">
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                ...
                                <table class="table table-striped table-hover" id="customersTable" style="margin-bottom: 0;">
                                        ...
                                        @if($users && $users->count() > 0)
                                            @foreach($users as $index => $user)
                                                <tr>
                                                    ...
                                                    <td>{{ $user->contact ?? ($user->customer->contact ?? $user->mob_num ?? 'N/A') }}</td>
                                                    <td>{{ $user->address ?? ($user->customer->address ?? 'N/A') }}</td>
                                                    ...
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                <a class="dropdown-item" href="javascript:;" onclick="large_modal('{{ $user->id }}','show_recent_orders')"  data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg">Recent Orders</a>
                                                                <a class="dropdown-item" href="javascript:;" onclick="custom_delete('/del_customer/{{ $user->id }}')"  data-bs-toggle="modal" data-bs-target=".bd-example-modal-sm">Delete</a>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
...
<style>
    ...
    /* Add more space between customer name and email */
...
function loadCustomers(page, limit, search) {
    ...
    // Build URL
    let url = "{{ route('customers') }}?page=" + page + "&limit=" + limit;
    ...
    .then(html => {
        ...
        // Extract table body
        const newTbody = tempDiv.querySelector('#customersTable tbody');
...
function attachPaginationListeners() {
    ...
    document.querySelectorAll('.pagination a.pagination-link').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            loadCustomers(page, currentLimit, currentSearch);
        });
    });
}
...
function changeEntriesPerPage() {
    ...
    loadCustomers(1, parseInt(limit), currentSearch);
}

function performSearch() {
    const search = document.getElementById('searchInput').value;
    loadCustomers(1, currentLimit, search);
}
