@extends('index')
@section('content')

<div class="content-body " style="min-height: 1100px;">
    <div class="container-fluid">
        <div class="row">
            <div class="col-xl-12">
                @include('layouts.flashmessage')
                <div class="card h-auto">
                    <div class="card-header p-0">
                        <div class="user-bg w-100">
                            <div class="user-svg">
                                <svg width="264" height="109" viewBox="0 0 264 109" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect x="0.0107422" y="0.6521" width="263.592" height="275.13" rx="20" fill="#FCC43E"></rect>
                                </svg>
                            </div>
                            <div class="user-svg-1">
                                <svg width="264" height="59" viewBox="0 0 264 59" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect y="0.564056" width="263.592" height="275.13" rx="20" fill="#FB7D5B"></rect>
                                </svg>

                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div class="user">
                                <div class="user-media">
                                    @php
                                    if ($shop->profile_photo){
                                        $image = url('/assets/images/profile/' . $shop->profile_photo);
                                    } else {
                                        $image = asset('assets/images/no-img-avatar.png');
                                    }
                                    @endphp
                                    <img src="{{$image}}" alt="" class="avatar avatar-xxl">
                                </div>
                                <div>
                                    <h2 class="mb-0">{{$shop->shopname}}</h2>
                                </div>
                            </div>
                            <div class="dropdown custom-dropdown">
                            <a href="javascript:;" onclick="large_modal('','createCategory/{{$shop->id}}','Add Category')" data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg" class="btn btn-outline-success">Add Category</a>
                            <button class="btn btn-outline-{{ $shop->status == 2 ? 'success' : 'danger' }}">{{$shop->status == 2 ? 'Active' : 'Inactive'}}</button>
                                {{-- <div class="btn sharp tp-btn " data-bs-toggle="dropdown">
                                    <svg width="24" height="6" viewBox="0 0 24 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M12.0012 0.359985C11.6543 0.359985 11.3109 0.428302 10.9904 0.561035C10.67 0.693767 10.3788 0.888317 10.1335 1.13358C9.88829 1.37883 9.69374 1.67 9.56101 1.99044C9.42828 2.31089 9.35996 2.65434 9.35996 3.00119C9.35996 3.34803 9.42828 3.69148 9.56101 4.01193C9.69374 4.33237 9.88829 4.62354 10.1335 4.8688C10.3788 5.11405 10.67 5.3086 10.9904 5.44134C11.3109 5.57407 11.6543 5.64239 12.0012 5.64239C12.7017 5.64223 13.3734 5.36381 13.8686 4.86837C14.3638 4.37294 14.6419 3.70108 14.6418 3.00059C14.6416 2.3001 14.3632 1.62836 13.8677 1.13315C13.3723 0.637942 12.7004 0.359826 12 0.359985H12.0012ZM3.60116 0.359985C3.25431 0.359985 2.91086 0.428302 2.59042 0.561035C2.26997 0.693767 1.97881 0.888317 1.73355 1.13358C1.48829 1.37883 1.29374 1.67 1.16101 1.99044C1.02828 2.31089 0.959961 2.65434 0.959961 3.00119C0.959961 3.34803 1.02828 3.69148 1.16101 4.01193C1.29374 4.33237 1.48829 4.62354 1.73355 4.8688C1.97881 5.11405 2.26997 5.3086 2.59042 5.44134C2.91086 5.57407 3.25431 5.64239 3.60116 5.64239C4.30165 5.64223 4.97339 5.36381 5.4686 4.86837C5.9638 4.37294 6.24192 3.70108 6.24176 3.00059C6.2416 2.3001 5.96318 1.62836 5.46775 1.13315C4.97231 0.637942 4.30045 0.359826 3.59996 0.359985H3.60116ZM20.4012 0.359985C20.0543 0.359985 19.7109 0.428302 19.3904 0.561035C19.07 0.693767 18.7788 0.888317 18.5336 1.13358C18.2883 1.37883 18.0937 1.67 17.961 1.99044C17.8283 2.31089 17.76 2.65434 17.76 3.00119C17.76 3.34803 17.8283 3.69148 17.961 4.01193C18.0937 4.33237 18.2883 4.62354 18.5336 4.8688C18.7788 5.11405 19.07 5.3086 19.3904 5.44134C19.7109 5.57407 20.0543 5.64239 20.4012 5.64239C21.1017 5.64223 21.7734 5.36381 22.2686 4.86837C22.7638 4.37294 23.0419 3.70108 23.0418 3.00059C23.0416 2.3001 22.7632 1.62836 22.2677 1.13315C21.7723 0.637942 21.1005 0.359826 20.4 0.359985H20.4012Z" fill="#A098AE"></path>
                                    </svg>
                                </div>
                                <div class="dropdown-menu dropdown-menu-end">
                                    <a class="dropdown-item" href="javascript:void(0);">Option 1</a>
                                    <a class="dropdown-item" href="javascript:void(0);">Option 2</a>
                                    <a class="dropdown-item" href="javascript:void(0);">Option 3</a>
                                </div> --}}
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-xl-3 col-xxl-6 col-sm-6">
                                <ul class="student-details">
                                    <li class="me-2">
                                        <a class="icon-box bg-secondary">
                                            <img src="{{ asset('assets/images/profile.svg') }}" alt="">
                                        </a>
                                    </li>
                                    <li>
                                        <span>Location:</span>
                                        <h5 class="mb-0">{{$shop->location}}</h5>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-xl-3 col-xxl-6 col-sm-6">

                                <ul class="student-details">
                                    <li class="me-2">
                                        <a class="icon-box bg-secondary">
                                            <img src="{{ asset('assets/images/svg/location.svg') }}" alt="">
                                        </a>

                                    </li>
                                    <li><span>Address:</span>
                                        <h5 class="mb-0">{{$shop->address}}</h5>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-xl-3 col-xxl-6 col-sm-6">
                                <ul class="student-details">
                                    <li class="me-2">
                                        <a class="icon-box bg-secondary">
                                            <img src="{{ asset('assets/images/svg/phone.svg') }}" alt="">
                                        </a>
                                    </li>
                                    <li><span>Phone:</span>
                                        <h5 class="mb-0">{{$shop->contact}}</h5>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-xl-3 col-xxl-6 col-sm-6">
                                <ul class="student-details">
                                    <li class="me-2">
                                        <a class="icon-box bg-secondary">
                                            <img src="{{ asset('assets/images/svg/email.svg') }}" alt="">
                                        </a>

                                    </li>
                                    <li><span>Email:</span>
                                        <h5 class="mb-0">{{$shop->email}}</h5>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>    
        <div class="row">
            <div class="col-xl-6">
                <div class="card h-auto">
                    <div class="card-body">
                        <h3 class="heading text-center">PRODUCT DETAILS</h3>
                    </div>
                </div>
                <?php 
                if ($category->isNotEmpty()) {?>
                <div class="card h-auto">
                    <div class="card-body p-4">
                        <?php foreach ($category as $c) {
                            $items = DB::table('products')->where('cat_id', $c->id)->where('shop_id', $c->shop_id)->get();?>
                        <div class="accordion accordion-active-header" id="accordion-{{$c->id}}">
                            <div class="accordion-item">
                                <div class="accordion-header  rounded-lg" id="accord-{{$c->id}}" data-bs-toggle="collapse" data-bs-target="#collapse{{$c->id}}" aria-controls="collapse{{$c->id}}"   @if($c->id == $category->first()->id) aria-expanded="true" @endif  role="button">
                                    <span class="accordion-header-icon"></span>
                                    <span class="accordion-header-text">{{$c->cat_name}} &nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="large_modal('','createItem/{{$shop->id}}/{{$c->id}}','Add Items')" data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg">
                                        <span class="btn-icon-start text-info">
                                            <i class="fa fa-plus color-info"></i>
                                        </span>
                                    </a>
                                </span>
                                    <span class="accordion-header-indicator"></span>
                                </div>
                                <div id="collapse{{$c->id}}" class="collapse accordion__body @if($c->id == $category->first()->id) show @endif" aria-labelledby="accord-{{$c->id}}" data-bs-parent="#accordion-{{$c->id}}">
                                    <div class="accordion-body-text">
                                        @php 
                                        if ($items->isNotEmpty()) {
                                            echo '<table class="table table-bordered">
                                            <thead>
                                                <tr>
                                                    <th scope="col">Product Name</th>
                                                    <th scope="col">Amount In KG</th>
                                                </tr>
                                            </thead>
                                            <tbody>';
                                            foreach ($items as $i) {
                                                echo '<tr>
                                                    <td>'.$i->name.'</td>
                                                    <td>'.$i->amout.'</td>
                                                </tr>';
                                            }
                                            echo '</tbody>
                                        </table>';
                                        } else {
                                            echo '<h3 class="heading text-center text-danger">PRODUCT NOT ADDED</h3>';
                                        }
                                        @endphp
                                    </div>
                                </div>
                            </div>
                        </div>
                        <?php }?>
                    </div>
                </div>
                <?php } else {?>
                    <div class="card h-auto">
                        <div class="card-body">
                            <h3 class="heading text-center text-danger">PRODUCT NOT ADDED</h3>
                        </div>
                    </div>
                <?php }?>
            </div>
            <div class="col-xl-6">
                {{-- <div class="row"> --}}
                    {{-- <div class="col-xl-12">
                        <div class="card h-auto">
                            <div class="card-body">
                                <h3 class="heading">Schedule Details</h3>
                                <p class="mb-0">Thursday, 10th April , 2022</p>
                            </div>
                        </div>
                    </div> --}}
                    {{-- <div class="col-xl-12"> --}}
                        <div class="card h-auto">
                            <div class="card-body">
                                <h3 class="heading text-center">DELIVERY BOYS</h3>
                            </div>
                        </div>
                    {{-- </div --}}
                    <div class="row">
                        
                        <?php 
                            if ($delivery_boy->isNotEmpty()) {
                                foreach ($delivery_boy as $val) {
                                    if ($val->profile_img){
                                        $dimage = url('/assets/images/deliveryboy/' . $val->profile_img);
                                    } else {
                                        $dimage = asset('assets/images/no-img-avatar.png');
                                    }?>
                            <div class="col-xl-6">
                            <div class="card h-auto schedule-card">
                                <div class="card-body">
                                    <a href="javascript:;"  onclick="large_modal({{$val->id}},'view_del_boy','Delivery Boy Details')" data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg" class="text-primary"><h4 class="mb-0">{{$val->name}}</h4></a>
                                    
                                    {{-- <p>Algorithm</p> --}}
                                    <div class="d-flex align-items-center justify-content-between">
                                        <div>
                                            <ul>
                                                <li class="mb-2">
                                                    <svg class="me-2" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M18 5.25H16.5V4.5C16.5 4.30109 16.421 4.11032 16.2803 3.96967C16.1397 3.82902 15.9489 3.75 15.75 3.75C15.5511 3.75 15.3603 3.82902 15.2197 3.96967C15.079 4.11032 15 4.30109 15 4.5V5.25H12.75V4.5C12.75 4.30109 12.671 4.11032 12.5303 3.96967C12.3897 3.82902 12.1989 3.75 12 3.75C11.8011 3.75 11.6103 3.82902 11.4697 3.96967C11.329 4.11032 11.25 4.30109 11.25 4.5V5.25H9V4.5C9 4.30109 8.92098 4.11032 8.78033 3.96967C8.63968 3.82902 8.44891 3.75 8.25 3.75C8.05109 3.75 7.86032 3.82902 7.71967 3.96967C7.57902 4.11032 7.5 4.30109 7.5 4.5V5.25H6C5.40326 5.25 4.83097 5.48705 4.40901 5.90901C3.98705 6.33097 3.75 6.90326 3.75 7.5V18C3.75 18.5967 3.98705 19.169 4.40901 19.591C4.83097 20.0129 5.40326 20.25 6 20.25H18C18.5967 20.25 19.169 20.0129 19.591 19.591C20.0129 19.169 20.25 18.5967 20.25 18V7.5C20.25 6.90326 20.0129 6.33097 19.591 5.90901C19.169 5.48705 18.5967 5.25 18 5.25ZM5.25 7.5C5.25 7.30109 5.32902 7.11032 5.46967 6.96967C5.61032 6.82902 5.80109 6.75 6 6.75H7.5V7.5C7.5 7.69891 7.57902 7.88968 7.71967 8.03033C7.86032 8.17098 8.05109 8.25 8.25 8.25C8.44891 8.25 8.63968 8.17098 8.78033 8.03033C8.92098 7.88968 9 7.69891 9 7.5V6.75H11.25V7.5C11.25 7.69891 11.329 7.88968 11.4697 8.03033C11.6103 8.17098 11.8011 8.25 12 8.25C12.1989 8.25 12.3897 8.17098 12.5303 8.03033C12.671 7.88968 12.75 7.69891 12.75 7.5V6.75H15V7.5C15 7.69891 15.079 7.88968 15.2197 8.03033C15.3603 8.17098 15.5511 8.25 15.75 8.25C15.9489 8.25 16.1397 8.17098 16.2803 8.03033C16.421 7.88968 16.5 7.69891 16.5 7.5V6.75H18C18.1989 6.75 18.3897 6.82902 18.5303 6.96967C18.671 7.11032 18.75 7.30109 18.75 7.5V9.75H5.25V7.5ZM18.75 18C18.75 18.1989 18.671 18.3897 18.5303 18.5303C18.3897 18.671 18.1989 18.75 18 18.75H6C5.80109 18.75 5.61032 18.671 5.46967 18.5303C5.32902 18.3897 5.25 18.1989 5.25 18V11.25H18.75V18Z" fill="#FB7D5B"></path>
                                                    </svg>
                                                    {{$val->dob}}
                                                </li>
                                                <li>
                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M17 1H7C5.9 1 5 1.9 5 3V21C5 22.1 5.9 23 7 23H17C18.1 23 19 22.1 19 21V3C19 1.9 18.1 1 17 1ZM17 19H7V5H17V19ZM12 21C11.45 21 11 20.55 11 20C11 19.45 11.45 19 12 19C12.55 19 13 19.45 13 20C13 20.55 12.55 21 12 21Z" fill="#FCC43E"/>
                                                      </svg>
                                                      
                                                    {{$val->phone}}
                                                </li>
                                            </ul>
                                        </div>
                                        <div>
                                            <img src="{{$dimage}}" class="avatar avatar-lg" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <?php } } else { ?>
                            <div class="col-xl-12">
                                <div class="card h-auto">
                                    <div class="card-body">
                                        <h3 class="heading text-center text-danger">No Delivery Boy Added</h3>
                                    </div>
                                </div>
                            </div
                        <?php  } ?>
                            
                        
                        {{-- <div class="col-xl-6">
                        <div class="card h-auto schedule-card">
                            <div class="card-body">
                                <h4 class="mb-0">Basic Algorithm</h4>
                                <p>Algorithm</p>
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        <ul>
                                            <li class="mb-2">
                                                <svg class="me-2" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M18 5.25H16.5V4.5C16.5 4.30109 16.421 4.11032 16.2803 3.96967C16.1397 3.82902 15.9489 3.75 15.75 3.75C15.5511 3.75 15.3603 3.82902 15.2197 3.96967C15.079 4.11032 15 4.30109 15 4.5V5.25H12.75V4.5C12.75 4.30109 12.671 4.11032 12.5303 3.96967C12.3897 3.82902 12.1989 3.75 12 3.75C11.8011 3.75 11.6103 3.82902 11.4697 3.96967C11.329 4.11032 11.25 4.30109 11.25 4.5V5.25H9V4.5C9 4.30109 8.92098 4.11032 8.78033 3.96967C8.63968 3.82902 8.44891 3.75 8.25 3.75C8.05109 3.75 7.86032 3.82902 7.71967 3.96967C7.57902 4.11032 7.5 4.30109 7.5 4.5V5.25H6C5.40326 5.25 4.83097 5.48705 4.40901 5.90901C3.98705 6.33097 3.75 6.90326 3.75 7.5V18C3.75 18.5967 3.98705 19.169 4.40901 19.591C4.83097 20.0129 5.40326 20.25 6 20.25H18C18.5967 20.25 19.169 20.0129 19.591 19.591C20.0129 19.169 20.25 18.5967 20.25 18V7.5C20.25 6.90326 20.0129 6.33097 19.591 5.90901C19.169 5.48705 18.5967 5.25 18 5.25ZM5.25 7.5C5.25 7.30109 5.32902 7.11032 5.46967 6.96967C5.61032 6.82902 5.80109 6.75 6 6.75H7.5V7.5C7.5 7.69891 7.57902 7.88968 7.71967 8.03033C7.86032 8.17098 8.05109 8.25 8.25 8.25C8.44891 8.25 8.63968 8.17098 8.78033 8.03033C8.92098 7.88968 9 7.69891 9 7.5V6.75H11.25V7.5C11.25 7.69891 11.329 7.88968 11.4697 8.03033C11.6103 8.17098 11.8011 8.25 12 8.25C12.1989 8.25 12.3897 8.17098 12.5303 8.03033C12.671 7.88968 12.75 7.69891 12.75 7.5V6.75H15V7.5C15 7.69891 15.079 7.88968 15.2197 8.03033C15.3603 8.17098 15.5511 8.25 15.75 8.25C15.9489 8.25 16.1397 8.17098 16.2803 8.03033C16.421 7.88968 16.5 7.69891 16.5 7.5V6.75H18C18.1989 6.75 18.3897 6.82902 18.5303 6.96967C18.671 7.11032 18.75 7.30109 18.75 7.5V9.75H5.25V7.5ZM18.75 18C18.75 18.1989 18.671 18.3897 18.5303 18.5303C18.3897 18.671 18.1989 18.75 18 18.75H6C5.80109 18.75 5.61032 18.671 5.46967 18.5303C5.32902 18.3897 5.25 18.1989 5.25 18V11.25H18.75V18Z" fill="#FB7D5B"></path>
                                                </svg>
                                                March 20, 2022
                                            </li>
                                            <li>
                                                <svg class="me-2 ms-1" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M10 0C8.02219 0 6.08879 0.58649 4.4443 1.6853C2.79981 2.78412 1.51809 4.3459 0.761209 6.17317C0.00433284 8.00043 -0.193701 10.0111 0.192152 11.9509C0.578004 13.8907 1.53041 15.6725 2.92894 17.0711C4.32746 18.4696 6.10929 19.422 8.0491 19.8079C9.98891 20.1937 11.9996 19.9957 13.8268 19.2388C15.6541 18.4819 17.2159 17.2002 18.3147 15.5557C19.4135 13.9112 20 11.9778 20 10C20 8.68678 19.7413 7.38642 19.2388 6.17317C18.7363 4.95991 17.9997 3.85752 17.0711 2.92893C16.1425 2.00035 15.0401 1.26375 13.8268 0.761205C12.6136 0.258658 11.3132 0 10 0V0ZM10 18C8.41775 18 6.87104 17.5308 5.55544 16.6518C4.23985 15.7727 3.21447 14.5233 2.60897 13.0615C2.00347 11.5997 1.84504 9.99113 2.15372 8.43928C2.4624 6.88743 3.22433 5.46197 4.34315 4.34315C5.46197 3.22433 6.88743 2.4624 8.43928 2.15372C9.99113 1.84504 11.5997 2.00346 13.0615 2.60896C14.5233 3.21447 15.7727 4.23984 16.6518 5.55544C17.5308 6.87103 18 8.41775 18 10C18 12.1217 17.1572 14.1566 15.6569 15.6569C14.1566 17.1571 12.1217 18 10 18V18Z" fill="#FCC43E"></path>
                                                    <path d="M13 9H11V5C11 4.73478 10.8946 4.48043 10.7071 4.29289C10.5196 4.10536 10.2652 4 10 4C9.73478 4 9.48043 4.10536 9.29289 4.29289C9.10536 4.48043 9 4.73478 9 5V10C9 10.2652 9.10536 10.5196 9.29289 10.7071C9.48043 10.8946 9.73478 11 10 11H13C13.2652 11 13.5196 10.8946 13.7071 10.7071C13.8946 10.5196 14 10.2652 14 10C14 9.73478 13.8946 9.48043 13.7071 9.29289C13.5196 9.10536 13.2652 9 13 9Z" fill="#FCC43E"></path>
                                                </svg>
                                                09.00 - 10.00 AM
                                            </li>
                                        </ul>
                                    </div>
                                    <div>
                                        <img src="/cakephp/demo/images/avatar/1.jpg" class="avatar avatar-lg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                        </div> --}}
                    </div>
                    {{-- </div> --}}
                {{-- </div> --}}
            </div>
        </div>
        </div>
    </div>		
</div>
@endsection